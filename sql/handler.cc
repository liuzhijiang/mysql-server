...
{
  sql_print_information("[%s:%d] enter handler::update_auto_increment", __FILE__, __LINE__);
  ...
  */
  ...
  sql_print_information("[%s:%d] nr: %lld, table->next_number_field->val_int(): %llu, (table->auto_increment_field_not_null: %d, thd->variables.sql_mode & MODE_NO_AUTO_VALUE_ON_ZERO: %lld)",
			__FILE__, __LINE__, nr, table->next_number_field->val_int(),
  ...
  sql_print_information("[%s:%d] nr: %lld", __FILE__, __LINE__, nr);

  sql_print_information("[%s:%d] next_insert_id: %lld, table->next_number_field->get_max_int_value(): %llu",
			__FILE__, __LINE__, nr, table->next_number_field->get_max_int_value());
  ...
  sql_print_information("[%s:%d] nr: %lld, next_insert_id: %lld, auto_inc_interval_for_cur_row.maximum(): %lld",
			__FILE__, __LINE__, nr, next_insert_id, auto_inc_interval_for_cur_row.maximum());
  if ((nr= next_insert_id) >= auto_inc_interval_for_cur_row.maximum())
      ...
      else
	{
	  ...
	  */
	  ...
	  get_auto_increment(variables->auto_increment_offset,
			     ...
			     &nb_reserved_values);
	  sql_print_information("[%s:%d] nb_desired_values: %lld, nr: %lld, nb_reserved_values: %lld", __FILE__, __LINE__, nb_desired_values, nr, nb_reserved_values);
	  if (nr == ULLONG_MAX)
      ...
      if (table->s->next_number_keypart == 0)
	{
	  ...
	  append= TRUE;
	  sql_print_information("[%s:%d] append: %d", __FILE__, __LINE__, append);
	}
      else
	{
	  /*
  ...
  */
  ...
  sql_print_information("[%s:%d] insert_id_for_cur_row: %lld", __FILE__, __LINE__, insert_id_for_cur_row);
  ...
  */
  ...
  sql_print_information("[%s:%d] next_insert_id: %lld", __FILE__, __LINE__, next_insert_id);
